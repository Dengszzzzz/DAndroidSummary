...
public class RxTransformActivity extends BaseActivity {
    ...
    @BindView(R2.id.tv_compose)
    TextView mTvCompose;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.rx_ac_transform);
        ButterKnife.bind(this);
        initTitle();
        tvTitle.setText("转化类操作符");
    }

    @OnClick({R2.id.btn_map, R2.id.btn_flatMap, R2.id.btn_concatMap, R2.id.btn_buffer, R2.id.btn_compose})
    public void onViewClicked(View view) {
        int i = view.getId();
        if (i == R.id.btn_map) {
            map();
        } else if (i == R.id.btn_flatMap) {
            flatMap();
        } else if (i == R.id.btn_concatMap) {
            concatMap();
        } else if (i == R.id.btn_buffer) {
            buffer();
        } else if (i == R.id.btn_compose) {
            compose();
        }
    }
