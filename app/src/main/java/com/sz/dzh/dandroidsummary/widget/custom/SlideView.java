...
import com.sz.dzh.dandroidsummary.utils.DensityUtils;


/**
 * 来源：https://github.com/Gnepux/SlideView
 *
 * 滑动按钮
 * 1.在ViewGroup 添加一个 TextView(前台) 和 一个 SlideIcon(滑动icon)
 * 2.在ViewGroup 滑动会绘制一个canvas
 */
public class SlideView extends ViewGroup {
    ...
    // attr: 背景文字-enable = true
    private String mBgTextEnableTrue = "";

    // attr: 背景文字-禁用时
    private String mBgTextEnableFalse = "";
    ...
    // attr: Icon背景图-禁用时
    private int mIconResIdEnableFalse;
    ...
    public void setDrawableBg(int resId){
        mBgResId = resId;
        setBackgroundResource(mBgResId);
    }

    public void setDrawableIcon(int resId){
        mIconResId = resId;
        mSlideIcon.setBackgroundResource(mIconResId);
    }
    ...
    public void setEnabled(boolean enabled) {
        ...
        mSlideIcon.setEnable(enabled);
        if (enabled){
            mSlideIcon.setBackgroundResource(mIconResId);
            mBgText = mBgTextEnableTrue;
        }else{
            mSlideIcon.setBackgroundResource(mIconResIdEnableFalse);
            mBgText = mBgTextEnableFalse;
        }
    }
    ...
    public SlideView(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mIconResIdEnableFalse = a.getResourceId(R.styleable.SlideView_icon_drawable_enable_false, android.R.drawable.btn_default);
            ...
            mBgTextEnableTrue = a.getString(R.styleable.SlideView_bg_text);
            mBgTextEnableFalse = a.getString(R.styleable.SlideView_bg_text_enable_false);
            mBgText = mBgTextEnableTrue;
    ...
    private void init() {
        ...
        LayoutParams layoutParams = new LayoutParams(600, LayoutParams.WRAP_CONTENT);
        mTextView.setLayoutParams(layoutParams);
        mTextView.setGravity(Gravity.CENTER);
        mTextView.setTextColor(mBgTextColor);
        mTextView.setTextSize(20);
        mTextView.setTypeface(Typeface.DEFAULT_BOLD);
        Log.d(TAG, "DensityUtils.dip2px(getContext(), 20) = " + DensityUtils.dip2px(getContext(), 20));
        addView(mTextView);
        ...
        mMotionListener = new MotionListener() {
            ...
            public void onActionUp(int x) {
                mIconX = x;
                mDistanceX = 0;
                if (mIconX + mSlideIcon.getMeasuredWidth() < getMeasuredWidth()) {  // SlideIcon未拖动到底
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        } else if (mIconX + mDistanceX + mSlideIcon.getMeasuredWidth() >= getMeasuredWidth()) { // 控制SlideIcon不能超过右边界限
